...
def beam_searching(
    ...
    for i, hypotheses in enumerate(generated_hyps):
        sorted_hyps = sorted(hypotheses.beams, key=lambda x: x[0], reverse=True)

        # if fewer than k (rare), pad with empty/padding hypothesis
        while len(sorted_hyps) < k:
            sorted_hyps.append((float("-inf"), torch.empty(0, dtype=torch.long)))

        for j in range(k):
            score, hyp = sorted_hyps[j]
            nbest_tokens.append(hyp.detach().clone())
            nbest_lens.append(int(hyp.numel()))
        ...
        for j in range(sent_per_beam):
            ...
            hypo_text_list.append(best_hyp.detach().clone())
            hypo_text_confid.append(float(_hypo[0]))

    # hypotheses.beams: list of (score, hyp_tensor)
    # print("num beams stored:", len(hypotheses.beams))
    # for r, (s, h) in enumerate(sorted(hypotheses.beams, key=lambda x: x[0], reverse=True)[:10]):
    #     print(r, float(s), h.tolist())

    nbest_lens = torch.tensor(nbest_lens, device=cuda_device, dtype=torch.long)
    nbest_scores = torch.tensor(nbest_scores, device=cuda_device, dtype=torch.float)

    # pad tokens
    max_len = max(nbest_lens.max().item(), 1)
    hypo_text = torch.full(
        (batch_size * k, max_len),
        padding_idx,
        dtype=torch.long,
        device=cuda_device,
    )
    # for j in range(k):
    #     print("selected", j, nbest_scores[j], nbest_tokens[j].tolist())

    for idx, hyp in enumerate(nbest_tokens):
        if len(hyp) > 0:
            hypo_text[idx, : len(hyp)] = hyp.to(cuda_device)

    # reshape to (B, K, T) etc.
    hypo_text = hypo_text.view(batch_size, k, -1)
    hypo_text_len = nbest_lens.view(batch_size, k)
    hypo_text_confid = nbest_scores.view(batch_size, k)
