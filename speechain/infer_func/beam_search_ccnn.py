...
def beam_searching(
    ...
    # --- CCNN parameters ---
    ccnn_weight: float = 0.0,
    ccnn_temperature: float = 1.0,
    ccnn_lm_temperature: float = 1.0,
    ccnn_cons_temperature: float = 2.0,
    ccnn_cons_beta: float = 0.5,
    ccnn_head: str = "lm",
    ccnn_decode_fn=None,
    # --- Other parameters ---
    sent_per_beam: int = 1,
    ...
    Args:
        ...
        ccnn_weight: float = 0.0
            The weight put on the CCNN scores at each decoding step.
        ccnn_temperature: float = 1.0
            The temperature coefficient used for calculating the log-softmax probability for the CCNN decoder.
        ccnn_lm_temperature: float = 1.0
            The temperature for the LM head of CCNN.
        ccnn_cons_temperature: float = 2.0
            The temperature for the constituency head of CCNN.
        ccnn_cons_beta: float = 0.5
            The weight for constituency logits in the mixed head.
        ccnn_head: str = "lm"
            The CCNN head to use: "lm", "cons", or "mix".
        ccnn_decode_fn = None
            The CCNN forward function for decoding the current partial hypothesis.
    ...
    # --- CCNN Arguments Validation --- #
    assert (
        0.0 <= ccnn_weight
    ), f"ccnn_weight should be a non-negative float number, but got ccnn_weight={ccnn_weight}!"
    assert (
        ccnn_temperature > 0.0
    ), f"ccnn_temperature should be a positive float number, but got ccnn_temperature={ccnn_temperature}"
    assert (
        ccnn_lm_temperature > 0.0
    ), f"ccnn_lm_temperature should be a positive float number, but got ccnn_lm_temperature={ccnn_lm_temperature}"
    assert (
        ccnn_cons_temperature > 0.0
    ), f"ccnn_cons_temperature should be a positive float number, but got ccnn_cons_temperature={ccnn_cons_temperature}"
    assert (
        ccnn_head in ["lm", "cons", "mix"]
    ), f"ccnn_head must be one of ['lm', 'cons', 'mix'], but got ccnn_head={ccnn_head}"
    ...
    while hypo_text_len.max() < hypo_maxlen:
        ...
        # --- 5.4. CCNN Forward --- #
        if ccnn_decode_fn is not None and ccnn_weight > 0:
            # Get current hypothesis lengths for CCNN
            ccnn_lengths = hypo_text_len

            # Prepare candidate tokens - use all vocabulary for LM head, or top-k for efficiency
            if ccnn_head == "lm":
                # For LM head, score all vocabulary tokens
                cand_ids = None
            else:
                # For cons/mix heads, use top-k candidates for efficiency
                topk_values, topk_indices = torch.topk(next_token_scores, k=min(50, vocab_size), dim=-1)
                cand_ids = topk_indices.unsqueeze(-1)  # (batch_size × beam_size, k, 1)

            # Forward pass through CCNN
            with torch.no_grad():
                lm_logits, cons_logits = ccnn_decode_fn(
                    x=hypo_text,
                    lengths=ccnn_lengths,
                    cand_ids=cand_ids
                )

                # Choose scoring head and apply temperature
                if ccnn_head == "lm":
                    ccnn_logits = lm_logits / max(ccnn_lm_temperature, 1e-6)
                elif ccnn_head == "cons":
                    ccnn_logits = cons_logits / max(ccnn_cons_temperature, 1e-6)
                else:  # mix
                    ccnn_logits = (lm_logits / max(ccnn_lm_temperature, 1e-6)) + \
                                 (ccnn_cons_beta * (cons_logits / max(ccnn_cons_temperature, 1e-6)))

                # Convert to log probabilities
                if cand_ids is not None:
                    # For candidate scoring, we need to map back to full vocabulary
                    ccnn_next_token_scores = torch.full_like(next_token_scores, float('-inf'))
                    ccnn_next_token_scores.scatter_(-1, topk_indices,
                                                   torch.log_softmax(ccnn_logits.squeeze(-1), dim=-1))
                else:
                    ccnn_next_token_scores = torch.log_softmax(ccnn_logits, dim=-1)

                # Apply CCNN weight and add to next token scores
                next_token_scores = next_token_scores + ccnn_weight * ccnn_next_token_scores
