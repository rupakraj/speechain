...
function print_help_message {
  ...
  $0 \\ (The arguments in [] are optional while other arguments must be given by your run.sh.)
      ...
      [--tgt_path TGT_PATH] \\                              # The metadata files will be generated to {tgt_path}/slr54nepaliasr. If tgt_path is not given, metadata files will be saved to ${SPEECHAIN_ROOT}/datasets/slr54nepaliasr. If you want to save metadata files elsewhere, please give its absolute path (starting by a slash '/') by this argument. (default: none)
      ...
      [--sample_rate SAMPLE_RATE] \\                        # The sampling rate you want the waveforms to have. If not given, the original sampling rate of slr54nepaliasr (16000Hz) will be used for the folder 'slr54nepaliasr/data/wav' (default: none)
      ...
      [--vocab_size VOCAB_SIZE] \\                          # The size of the tokenizer vocabulary. #TODO: check the impact
      ...
      [--separator SEPARATOR] \\                            # The separator used to separate the 'subsets' arguments from a string into an array of string. (default: ',')" >&2
  exit 1
}
...
# There is no official subsets in OSLR54 dataset, so the names are simply set to 'train', 'valid', and 'test' here.
